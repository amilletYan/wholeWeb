    ...
    jPlayerPlaylist.prototype = {
        ...
        _createItemHandlers: function() {
            ...
            a(this.cssSelector.playlist).off("click", "a." + this.options.playlistOptions.itemClass).on("click", "a." + this.options.playlistOptions.itemClass, function(c) {
                    ...
                    if(xmlhttp.readyState==4&&xmlhttp.status==200){
                        var data=xmlhttp.responseText;
                         ...
                         for(let i=0;i<lines.length;i++){
                          var matchs=lines[i].match(reg);
                         // var matchs=reg.exec(lines[i]);
                          if(matchs){
                              var minute=parseFloat(matchs[1]);
                              var seconds=parseFloat(matchs[2]);
                              var millisecond=parseFloat(matchs[3]);
                              var words=matchs[4];
                              //let就能解决words变量提升问题，在每个块级作用域都存在一个words
                              //而用var，只在function函数中存在一个words
                              var time=minute*60*1000+seconds*1000+millisecond;
                              setTimeout((function(words){
                                document.getElementById("lrc").innerText=words  
                              }).bind(this,words),time);
                          }
                         }
                    }
                }
                xmlhttp.open("GET",lrcpath);
                xmlhttp.send();
                };
                c.preventDefault();
                ...
                if(b.current !== d ){
                    b.play(d);
                    document.getElementsByClassName("pic_info")[0].children[0].innerText=b.playlist[d].title; 
                    document.getElementsByClassName("pic-sec")[0].children[0].children[0].setAttribute("src",b.playlist[d].pic);
                    loadLrc(b.playlist[d].lrc);


                }
               else{a(b.cssSelector.jPlayer).jPlayer("play"),
                b.blur(this);}
